<?php
	...
	if ($template->bimestre > 0) {
		$query = "select alunos.id, alunos.nome, alunos.matricula from disciplinas inner join alunos on(alunos.disciplina_id = disciplinas.id) where disciplinas.id = $1";
	} else {		
...
where disciplinas.id = $1 group by alunos.id, alunos.matricula, alunos.nome
...
having sum(obtido) < 28 order by alunos.id"; 
	...
	while ($registro = pg_fetch_array($result)){
		...
		$sql = "select obtido from notas where aluno_id = $1 and avaliacao_id = $2";
